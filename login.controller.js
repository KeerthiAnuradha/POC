...
angular.module('hillromvestApp')
  .controller('LoginController',['$scope','$state', '$cookies', '$cookieStore', 'deviceDetector', '$timeout', 'Auth', 'vcRecaptchaService', 'globalConfig', '$rootScope', 'loginConstants', 'Principal', 'StorageService', 'patientsurveyService', 'Account','$window',
    function($scope, $state, $cookies, $cookieStore, deviceDetector, $timeout, Auth, vcRecaptchaService, globalConfig, $rootScope, loginConstants, Principal, StorageService, patientsurveyService, Account,$window) {
  ...
  //console.log( $scope.dynamicHeightObj );
  $('#myCarousel').carousel({
            interval:6500,
           pause: "false"
      });
  ...
  $scope.onCarouselInit = function() {
      ...
      Auth.login({
        ...
        if (response.status === 200) { 
          ...
          localStorage.setItem('deviceTypeforBothIcon_'+response.data.user.id, 'ALL');
            ...
            logged.userId = response.data.user.id;
             $scope.getPatientListForCaregiver(logged.userId,true,'caregiverDashboard');
          ...
          } else if( response.data.user.authorities[0].name === 'CLINIC_ADMIN' || response.data.user.authorities[0].name === 'ASSOCIATE_EXECUTIVE' ){
            logged.userId = response.data.user.id;
            StorageService.save('logged', logged);
            $scope.getisMessagesOpted();
            $state.go('clinicadmindashboard');
        ...
        if (response.status === 401) {
          ...
          $scope.authenticationError = true;
          $scope.message = "The site is currently unavailable. Please try again later."
        }
      });
    };
    ...
    $scope.login = function(event) {
      ...
      if ($scope.showCaptcha) {
        ...
        $cookieStore.put('userName', $scope.username, { domain: '.hillromvest.com' });
        $cookieStore.put('password', $scope.password, { domain: '.hillromvest.com' });
        console.log("cookie1",$cookieStore.get('userName'));
        console.log("cookie1",$cookieStore.get('password'));
